...
checkAuth = function(socket, next) {
	...
	query.on('result', function(row) {
		  console.log(`UCP session ${suppliedToken} found for ${address}`);
			freepbx.db.query (`UPDATE ucp_sessions SET socketid = '${socket.id}' WHERE session = '${suppliedToken}' AND address = '${address}'`);
			auth = true;
	}).on('end', function() {
		if (auth) {
