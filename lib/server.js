...
checkAuth = function(socket, next) {
	...
	var query = freepbx.db.query(`SELECT * FROM ucp_sessions WHERE session = '${suppliedToken}' AND address = '${address}'`);
	query.on('result', function(row) {
			const updateSQL = 'UPDATE ucp_sessions SET socketid = :socketid WHERE session = :session AND address = :address';
			freepbx.db.execute(updateSQL, { session: suppliedToken, address: address, socketid: socket.id }, (err, result, fields) => {
